...
const SettingsAreaEquivalence: FC<SettingsAreaEqProps> = ({buildArea, setBuildArea, createArea}) => {
  ...
  const removeLevel = (idx: number) => {
    setBuildArea({...buildArea, level:
    buildArea.level.filter((_, i) => i !== idx)})
  }
  const updateLevel = (idx: number, value?: number) => {
    if (value !== undefined) {
      setBuildArea({...buildArea, level:
      buildArea.level.map((v, i) => {
        if (idx === i)
          return value
        return v
      })})
  ...
  return (
    <div className={styles.container}>
      <div className={styles.scroll} ref={scrollRef}>
        ...
        <h1 className={styles.title}>
          Частота сетки для расчёта
        </h1>
        <div className={styles.sliderContainer}>
          <input
            value={buildArea.range}
            onChange={(e) => setBuildArea(
              {...buildArea, range: Number(e.target.value)})}
            type='range'
            max='50'
            min='5'
            step='1'
